...
# -- انپین کردن --
@Client.on_message(
    filters.command("unpin") | filters.regex(r"^ا+ن+پ+ی+ن$") & filters.group)
async def unpin_msg(c, m: Message):
    chat_id = str(m.chat.id).strip('-')
    if get_admins(chat_id, m.from_user.id):
        await m.reply_to_message.unpin()
        await m.reply_text("پیام انپین شد!",
                           reply_to_message_id=m.reply_to_message.message_id)


# -- پین کردن --
@Client.on_message(
    filters.command("pin") | filters.regex(r"^پ+ی+ن$")
    | filters.regex(r"^📌") & filters.group)
...
async def welcome(c: Client, m: Message):
    ...
    if m.new_chat_members[0]["id"] == 1886243847:
        ...
        if users is None:
            ...
            cur.execute(
                "UPDATE USERS SET status=(?), is_admin=(?) WHERE main.USERS.num_id = (?)",
    ...
    روی دکمه ی زیر کلیک کن تا بفهمیم ربات نیستی.""",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🤖من ربات نیستم🤖",
...
async def left(c, m: Message):
    ...
    cur = con.cursor()
    cur.execute(
        "UPDATE USERS SET 'status' = 'left', is_admin=0 where num_id=:id",
        {'id': num_id})
    con.commit()
    # print("Commited!")
    ...
    await m.reply_text(
        f"رفتی [{m.from_user.first_name}](tg://user?id={m.from_user.id})\n حوالت پا چراغ نفتی "
    )
...
""")
                ...
                cur.execute(
                    "UPDATE USERS SET warn =(?) WHERE num_id=(?)",
                    (warns[0] + 1, num_id),
                )
...
@Client.on_message(
    filters.command("del_warns")
    | filters.regex(r"^ح+ذ+ف+ +ا+خ+ط+ا+ر$") & filters.group)
    ...
    if get_admins(str(m.chat.id).strip("-"), m.from_user.id):
        if m.reply_to_message is not None:
            ...
            if warns[0] == 0:
                ...
                cur.execute("UPDATE USERS SET warn=(?) where num_id=(?)", (
                    warns[0] - 1,
                    num_id,
                ))
                con.commit()
...
@Client.on_message(
    filters.command('mute') | filters.regex(r"^م+ی+و+ت") & filters.group)
...
""")
                ...
                await c.restrict_chat_member(chat_id, user_id,
                                             ChatPermissions(),
                                             int(utime() + m2s(minutes)))
                await m.reply_to_message.reply_text(
                    f"""[{m.reply_to_message.from_user.first_name}](tg://user?id={user_id})
شما به مدت {minutes} دقیقه میوت شدید😜
                """)
            elif m.command[2] == ("minutes"
                                  or "min") and m.command is not None:
                minutes = m.command[1]
                minutes = int(minutes)
                await c.restrict_chat_member(chat_id, user_id,
                                             ChatPermissions(),
...
@Client.on_message(
    filters.command("unmute") | filters.regex(r"^ا+ن+م+ی+و+ت")
    | filters.regex(r"^ح+ذ+ف+ +س+ک+و+ت$") & filters.group)
...
@Client.on_message(
    filters.command("ban") | filters.regex(r"^ب+ن$") & filters.group)
async def ban(c: Client, m: Message):
    if get_admins(str(m.chat.id).strip("-"), m.from_user.id):
        ...
        con = sqlite3.connect("databases/" + dbname)  # وصل شدن به دیتا بیس
        cur = con.cursor()
        cur.execute("UPDATE USERS SET 'status' = 'banned' where num_id=:id",
                    {'id': user_id})
        con.commit()
        await c.kick_chat_member(chat_id, user_id)
        await m.reply_text(
            f"@{m.reply_to_message.from_user.username} برای همیشه از گروه بن شد😃"
        )
        con.close()
...
# -- حذف پیام --
@Client.on_message(
    filters.command("del") | filters.regex(r"^حذف") & filters.group)
...
async def set_admin(c: Client, m: Message):
    ...
    if get_admins(str(chat_id).strip("-"), m.from_user.id):
        if not get_admins(
                str(chat_id).strip("-"), m.reply_to_message.from_user.id):
            dbname = str(m.chat.id).strip("-") + '.db'
            # user_id = m.reply_to_message.from_user.id
            # chat_id = m.chat.id
            # print("test1")
            ...
            cur = con.cursor()
            cur.execute(
                "UPDATE USERS SET status = (?) , is_admin=(?)  WHERE num_id = (?)",
                ('administrator', True, user_id))
            con.commit()
            await c.promote_chat_member(chat_id, user_id, False, True, True,
...
@Client.on_message(
    filters.command('^why') | filters.regex(r"^چ+ر+ا.+")
    | filters.regex(r'^چ+ط+و+ری.+') | filters.regex(r"^چ+گ+و+ن+ه.+")
    | filters.regex(r'^چ+ج+و+ر+ی.+') & filters.group)
async def button(c: Client,
                 m: Message):  # Callback Query handler is in the main.py
...
async def search(c: Client, m: Message):
    ...
    if len(m.command) == 1:
        ...
        if m.reply_to_message is None:  # check to see if message replied
            ...
            await m.reply_text('جواب سرچ شما در اینترنت:',
                               reply_markup=InlineKeyboardMarkup([
                                   [
                                       InlineKeyboardButton('داک داک گو',
                                                            url=url_ddg),
                                       InlineKeyboardButton('گوگل',
                                                            url=url_ggl)
                                   ],
                               ]))
        else:
            ...
            await m.reply_to_message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton('توی داک داک گو', url=url_lmddg),
                    InlineKeyboardButton('توی گوگل', url=url_bmbgk)
                ]]))


@Client.on_message(filters.command("echo") | filters.regex(r'^ا+ک+و')
                   )  # to bot speak instead of you (:
async def echo(c: Client, m: Message):
    if get_admins(str(m.chat.id).strip('-'), m.from_user.id):
        ...
        if m.command is None and m.reply_to_message is not None:
            ...
            msg = " ".join(m.command[1:])
            await m.reply_to_message.reply_text(
                f"[{msg}](tg://user?id={m.reply_to_message.from_user.id})")
...
۲- اگر فردی باعث آزرده خاطر شدن دیگر اعضا شود به وسیله نظر سنجی مجازات سطح ۳ یا ۴ برای او لحاظ می‌شود.
۳- اگر فردی بیش‌تر از ۲۵ پیام در کم‌تر از ۳ دقیقه بدهد و یا محتوایی بی مفهوم را چندین بار ارسال کند، اسپمر محسوب و بسته به تصمیم مدیران مطابق مجازات سطح ۱ یا ۲ با او برخورد می‌شود.

سطح‌بندی محتوای توهین‌آمیز:
⚫️ سطح ۱: توهین مستقیم به فامیل درجه اوّل افراد دیگر
🟤 سطح ۲: توهین غیر مستقیم به فامیل درجه اوّل افراد دیگر
🔴 سطح ۳: توهین مستقیم به خود افراد یا چیزهای دیگری که به ایشان مربوط می‌شود (خویشاوندان، قومیت، ملیت و مذهب نیز شامل این سطح می‌شود)
🟠 سطح ۴: توهین غیر مستقیم به خود افراد یا چیزهای دیگری که به ایشان مربوط می‌شود (خویشاوندان، قومیت، ملیت و مذهب نیز شامل این سطح می‌شود)

سطح‌بندی محتوای نامناسب:
(در صورتی که برخی موارد زیر را نمی‌دانید، می‌تونادی از موتورهای جستجو (مثل: DuckDuckGo, MetaGear, YaCy, searX) استفاده کنید)
⚫️ سطح ۱: پورن (Pornographic)
🟤 سطح ۲: محتوای ترسناک، به ویژه جامپ اسکیر (Jump scare)
🔴 سطح ۳: سایر چیزهای چندش‌آور یا نفرت‌انگیز (تبصره: استثنائاً در این مورد موارد غیرواقع‌گرایانه (کارتون، نقّاشی و...) مانند ایموجی «🤢» شامل نمی‌شود.)

برخوردها و مجازات:
(اخراج یا میوت برای همهٔ پلتورم‌های متّصل به هم اعمال می‌شود.)
⬛️ سطح ۱: تصمیم‌گیری میان اخراج دائم یا موقّت از گروه به وسیلهٔ نظرسنجی
🟫 سطح ۲: تصمیم‌گیری میان اخطار یا اخراج موقّت از گروه به وسیلهٔ نظرسنجی
🟥 سطح ۳: تصمیم‌گیری میان اخطار یا میوت موقّت از گروه به وسیلهٔ نظرسنجی
🟧 سطح ۴: میوت بسته به تصمیم مدیران

نحوه برگزاری نظر سنجی:
